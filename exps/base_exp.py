...
from exp_engine.accelerators import BaseAccelerator
...
from exp_engine.utils.log_utils import setup_logger
...
class BaseExp:
            ...
            if "exp" in hydra_cfg:
                for k, v in hydra_cfg.exp.items():
                    if hasattr(cfg, k):
                        ...
                        raise ValueError(f"CFG attribute {k} is not found")
    ...
    @cached_property
    def logger(self):
        distributed_rank = self.accelerator.process_index if self.accelerator else 0
        logger = setup_logger(self.output_dir, distributed_rank=distributed_rank, filename="log.log")
